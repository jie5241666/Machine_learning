...
def loss(w,b,X,y,lambd):
    '''带正则化项'''
    return np.mean((np.dot(w,X.T)+b -y)**2)/2+lambd*np.sum(w)/2
def grad(w,b,X,y,lambd):
    grad_w = np.mean((np.dot(w,X.T)+b-y).T*X,axis=0)+lambd*w
...
for i in range(100000):
    if i%5000==0: print(loss(w,b,train_x,train_y,lambd))
    grad_w,grad_b = grad(w,b,train_x,train_y,lambd)
